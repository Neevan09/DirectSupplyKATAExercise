...
function* handleGetPets(action) {
  yield put({ type: ActionTypes.API_FETCH });
console.log("action===============",action);
  try {
    const apiResponse = yield call(getPets, action.request);
    ...
    data
      ? yield put({
          type: ActionTypes.GET_PETS_RESPONSE_RECEIVED,
          response: data,
        })
      : yield put({ type: ActionTypes.GET_PETS_ERROR_RECEIVED });
  } catch (error) {
...
export default function* mySaga() {
  yield takeLatest(ActionTypes.LOAD_GET_PETS, handleGetPets);
}
